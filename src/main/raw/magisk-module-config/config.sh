...
extract_files() {
	...
	mkdir -p "$TMPDIR/vpnhide/"
	unzip -o "$ZIP" 'loader/*' -d "$TMPDIR/vpnhide/" > /dev/null 2> /dev/null
	if [[ "$ARCH" = "arm" ]];then
		mkdir -p "$MODPATH/system/lib"
		cp "$TMPDIR/vpnhide/loader/armeabi-v7a/libriru_vpnhide.so" "$MODPATH/system/lib/"
	elif [[ "$ARCH" = "arm64" ]];then
		mkdir -p "$MODPATH/system/lib"
		mkdir -p "$MODPATH/system/lib64"
		cp "$TMPDIR/vpnhide/loader/armeabi-v7a/libriru_vpnhide.so" "$MODPATH/system/lib/"
		cp "$TMPDIR/vpnhide/loader/arm64-v8a/libriru_vpnhide.so"   "$MODPATH/system/lib64/"
	else
	...
	unzip -o "$ZIP" 'injector.jar' -d "$MODPATH/data" > /dev/null 2> /dev/null
